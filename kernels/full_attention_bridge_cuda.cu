...
template <typename T>
int full_attention_decode_flat_device(
    int device_ordinal,
    int batch_size,
    int q_heads,
    int kv_heads,
    ...
    const int block = 32;
    if (head_dim > block * 8) return 24;
    const int rows = batch_size * q_heads;
    hipLaunchKernelGGL(
        HIP_KERNEL_NAME(supersonic_qwen35_full_attention_decode_flat_kernel<T>),
        dim3(rows),
        dim3(block),
        0,
        0,
...
int full_attention_decode_flat_strided_device(
    ...
    const int rows = batch_size * q_heads;
    hipLaunchKernelGGL(
        HIP_KERNEL_NAME(supersonic_qwen35_full_attention_decode_flat_strided_kernel<T>),
        dim3(rows),
        dim3(block),
        0,
        0,
...
extern "C" int supersonic_qwen35_hip_full_attention_decode_flat(
    ...
    switch (dtype) {
    case 0:
        return full_attention_decode_flat_device<half>(
            static_cast<int>(device_ordinal),
            static_cast<int>(batch_size),
            static_cast<int>(q_heads),
            static_cast<int>(kv_heads),
            ...
            out);
    case 2:
        return full_attention_decode_flat_device<hip_bfloat16>(
            static_cast<int>(device_ordinal),
            static_cast<int>(batch_size),
            static_cast<int>(q_heads),
            static_cast<int>(kv_heads),
...
extern "C" int supersonic_qwen35_hip_full_attention_decode_flat_strided(
    int dtype,
    size_t device_ordinal,
    size_t batch_size,
    size_t q_heads,
...
int persistent_decode_device(
    ...
    hipLaunchKernelGGL(
        HIP_KERNEL_NAME(supersonic_qwen35_persistent_decode_kernel<T, HERO_MODE>),
        dim3(static_cast<unsigned int>(num_blocks)),
        dim3(block_size),
        shared_bytes,
        0,
...
extern "C" int supersonic_qwen35_cuda_persistent_decode_qwen08_sm86_specialized(
    int dtype,
    size_t device_ordinal,
    size_t num_layers,
    size_t hidden_dim,
