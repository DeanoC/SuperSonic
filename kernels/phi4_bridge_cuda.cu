// Phi-4-mini persistent decode megakernel CUDA bridge.
//
...
// `dtype` encoding matches the Qwen/Gemma bridges: 0 = half, 2 = __nv_bfloat16.
// Only BF16 is wired at launch; FP16 returns 256 and is held for future work.
extern "C" int phi4_cuda_persistent_decode(
    int dtype,
...
extern "C" int phi4_cuda_rms_norm(
    int dtype,
    size_t device_ordinal,
    size_t n_rows,
    size_t n_cols,
    ...
    constexpr int block_size = 256;
    hipLaunchKernelGGL(
        HIP_KERNEL_NAME(phi4_rms_norm_kernel<__nv_bfloat16, false>),
        ...
        eps,
        static_cast<const __nv_bfloat16*>(xs),
