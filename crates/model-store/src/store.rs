...
fn validate_stage3_int4_binding_names(
    logical_name: &str,
    packed_name: &str,
    scale_name: &str,
    zero_name: Option<&str>,
) -> Result<(), Error> {
    if logical_name == scale_name || zero_name == Some(logical_name) {
        return Err(Error::Other(format!(
            "FLM Stage 3 INT4 tensor {logical_name} logical name must not equal SCALE or ZERO binding names"
        )));
    }
    if packed_name == scale_name || zero_name == Some(packed_name) || zero_name == Some(scale_name)
    {
        return Err(Error::Other(format!(
            "FLM Stage 3 INT4 tensor {logical_name} physical binding names must be pairwise distinct"
        )));
    }
    Ok(())
}
...
fn build_stage3_int4_storage(
    ...
    let mut staged_synthetic = synthetic.clone();
...
fn build_stage3_int4_storage_staged(
    ...
    index: &mut HashMap<String, TensorMeta>,
    upload_views: &mut HashMap<String, TensorUploadView>,
    _ct_int4_bf16_fallbacks: &mut HashMap<String, CtSymInt4Bf16Fallback>,
    int4_storage_views: &mut HashMap<String, Int4StorageView>,
    synthetic: &mut HashMap<String, Vec<u8>>,
    index_entries: &HashMap<String, FlmIndexEntry>,
    insert_logical_aliases: bool,
    ...
    for logical in runtime.logical_tensors() {
        ...
        validate_stage3_int4_binding_names(
            &logical.name,
            &packed.tensor_name,
            &scale.tensor_name,
            zero.map(|binding| binding.tensor_name.as_str()),
        )?;
        ...
        let mut scale_meta = index.get(&scale.tensor_name).cloned().ok_or_else(|| {
            Error::Other(format!(
        ...
        if kind == crate::flm::FlmStage3DirectWeightKind::CtInt4Bf16Fallback {
            ...
            if logical.rank != 2 {
                return Err(Error::Other(format!(
                    "FLM Stage 3 INT4 tensor {} rank {} is unsupported for CT direct INT4",
                    logical.name, logical.rank
                )));
            }
            if scale.storage_dtype != FLM_DTYPE_BF16 {
                return Err(Error::Other(format!(
                    "FLM Stage 3 INT4 tensor {} scale binding dtype {} is unsupported for CT direct INT4",
            ...
            {
                return Err(Error::Other(format!(
                    "FLM Stage 3 INT4 tensor {} uses unsupported CT ABI codec={} bits={}",
                    logical.name, storage_abi.codec_semantic_id, storage_abi.bits
                )));
            }
            if packed_step.target_dtype != FLM_DTYPE_UINT8 {
                return Err(Error::Other(format!(
                    "FLM Stage 3 CT INT4 tensor {} packed plan must target UINT8, got {}",
                    logical.name, packed_step.target_dtype
                )));
            }
            let group_size = usize::from(storage_abi.group_size);
            if group_size == 0 {
                return Err(Error::Other(format!(
                    "FLM Stage 3 CT INT4 tensor {} has zero group size",
                    logical.name
                )));
            }
            ...
            let expected_packed = vec![logical_shape[0], logical_shape[1].div_ceil(2)];
            let expected_scale = vec![logical_shape[0], logical_shape[1].div_ceil(group_size)];
            let expected_packed_len = expected_packed
                .iter()
                .try_fold(1u64, |acc, dim| acc.checked_mul(*dim as u64))
                .ok_or_else(|| {
                    Error::Other(format!(
                        "FLM Stage 3 CT INT4 tensor {} packed byte_len overflows",
                        logical.name
                    ))
                })?;
            if packed_shape != expected_packed || packed_meta.byte_len != expected_packed_len {
                return Err(Error::Other(format!(
                    "FLM Stage 3 CT INT4 tensor {} packed plan shape {:?} / stored {} does not match UINT8 view {:?} ({expected_packed_len} bytes)",
                    logical.name, packed_shape, packed_meta.byte_len, expected_packed
                )));
            }
            let expected_scale_len = expected_scale
                .iter()
                .try_fold(2u64, |acc, dim| acc.checked_mul(*dim as u64))
                .ok_or_else(|| {
                    Error::Other(format!(
                        "FLM Stage 3 CT INT4 tensor {} scale byte_len overflows",
                        logical.name
                    ))
                })?;
            if scale_step.target_dtype != FLM_DTYPE_BF16
                || scale_shape != expected_scale
                || scale_meta.byte_len != expected_scale_len
            {
                return Err(Error::Other(format!(
                    "FLM Stage 3 CT INT4 tensor {} scale plan dtype {} / shape {:?} / stored {} does not match BF16 view {:?} ({expected_scale_len} bytes)",
                    logical.name, scale_step.target_dtype, scale_shape, scale_meta.byte_len, expected_scale
                )));
            }
            if insert_logical_aliases {
                ...
                synthetic
                    .entry(zero_alias)
                    .or_insert_with(|| bf16_eight_bytes(expected_scale_len as usize));
            }
            int4_storage_views.insert(
                logical.name.clone(),
                Int4StorageView {
                    kind: Int4StorageKind::CtSymmetric,
                    group_size,
                    packed_tensor: packed.tensor_name.clone(),
                    scale_tensor: scale.tensor_name.clone(),
                    zero_tensor: None,
                    logical_shape,
                    packed_row_stride_bytes: expected_packed[1],
                    packed_expert_stride_bytes: expected_packed[0]
                        .saturating_mul(expected_packed[1]),
                    scale_row_stride_elements: expected_scale[1],
                    scale_expert_stride_elements: expected_scale[0]
                        .saturating_mul(expected_scale[1]),
                    output_group_size: 1,
        ...
        if kind == crate::flm::FlmStage3DirectWeightKind::RowGroupInt4 {
            ...
            let scale_codec = runtime
                .codec_by_id(u16::from(scale_entry.codec))
                .ok_or_else(|| {
                    Error::Other(format!(
                        "FLM Stage 3 scale tensor {} references missing codec id {}",
                        scale.tensor_name, scale_entry.codec
                    ))
                })?;
            if scale_codec.semantic_id as u16 != crate::flm::CODEC_RAW_BF16
                || scale_codec.layout_id != crate::flm::LAYOUT_ID_DEFAULT
                || scale_codec.decoder_id != 0
                || scale_codec.flags != 0
            {
                return Err(Error::Other(format!(
                    "FLM Stage 3 row-group scale tensor {} must reference a raw BF16 codec descriptor, got local id {} semantic={} layout={} decoder={} flags={}",
                    scale.tensor_name,
                    scale_entry.codec,
                    scale_codec.semantic_id,
                    scale_codec.layout_id,
                    scale_codec.decoder_id,
                    scale_codec.flags
                )));
            }
            scale_meta.dtype = "bf16".to_string();
            index
                .get_mut(&scale.tensor_name)
                .ok_or_else(|| {
                    Error::Other(format!(
                        "FLM Stage 3 scale tensor {} disappeared from the index",
                        scale.tensor_name
                    ))
                })?
                .dtype = "bf16".to_string();
...
impl BakedStore {
    ...
    pub fn open_flm_with_options(path: &Path, options: FlmLoadOptions) -> Result<Self, Error> {
        ...
        if let Some(runtime) = runtime.as_ref() {
            ...
            } else if options.flm_int4_logical_aliases {
...
mod tests {
    ...
    struct TestRowGroupStage3 {
        ...
        scale_index_codec: u8,
        scale_codec_semantic_id: u8,
        scale_codec_layout_id: u16,
        scale_codec_decoder_id: u16,
    ...
    impl TestRowGroupStage3 {
        fn rank2() -> Self {
            Self {
                logical_name: TEST_ROW_GROUP_LOGICAL_NAME,
                packed_name: TEST_ROW_GROUP_PACKED_NAME,
                scale_name: TEST_ROW_GROUP_SCALE_NAME,
                zero_name: TEST_ROW_GROUP_ZERO_NAME,
                rank: 2,
                ...
                scale_index_codec: 0,
                scale_codec_semantic_id: crate::flm::CODEC_RAW_BF16 as u8,
                scale_codec_layout_id: crate::flm::LAYOUT_ID_DEFAULT,
                scale_codec_decoder_id: 0,
                scale_codec_flags: 0,
    ...
    fn runtime_row_group_codec_section(fixture: &TestRowGroupStage3) -> Vec<u8> {
        ...
        put_u16(&mut out, record_offset + 4, fixture.scale_codec_decoder_id);
        put_u32(&mut out, record_offset + 6, fixture.scale_codec_flags);
        out
    }
    ...
    fn build_test_runtime_directory_with_row_group_stage3_tables(
        ...
        let sections = [
            (1u32, runtime_config_section()),
            (2u32, runtime_tokenizer_section()),
            (3u32, runtime_row_group_codec_section(fixture)),
            (4u32, runtime_tensor_abi_section()),
            (5u32, asset_table),
            (6u32, asset_payloads),
            (7u32, runtime_model_descriptor_section()),
    ...
    fn build_test_flm_with_stage3_row_group_bindings(fixture: &TestRowGroupStage3) -> Vec<u8> {
        let (packed_shape, scale_shape) = fixture.physical_shapes();
        let scale_payload_shape = fixture
            .scale_payload_shape
            .clone()
            .unwrap_or_else(|| scale_shape.clone());
        let (packed_elements, scale_elements) = fixture.payload_len_override.unwrap_or_else(|| {
            (
                packed_shape.iter().map(|dim| *dim as usize).product(),
                scale_payload_shape
                    .iter()
                    .map(|dim| *dim as usize)
                    .product(),
    ...
    #[test]
    fn open_flm_builds_ct_int4_stage3_binding_as_direct_int4() {
        ...
        let alias = store
            .meta("model.language_model.layers.0.mlp.gate_proj.weight")
            .expect("logical CT INT4 packed alias");
        assert_eq!(alias.layout, LayoutTag::Int4Quantized);
        assert_eq!(alias.dtype, "u8");
        assert_eq!(alias.shape, vec![128, 32]);
        assert_eq!(alias.byte_len, 128 * 32);

        let upload_view = store
            .upload_view("model.language_model.layers.0.mlp.gate_proj.weight")
            .expect("logical CT INT4 upload view");
        assert_eq!(upload_view.dtype, "u8");
        assert_eq!(upload_view.shape, vec![128, 32]);

        let scale = store
            .meta("model.language_model.layers.0.mlp.gate_proj.weight_int4_scale")
            .expect("CT INT4 scale alias");
        assert_eq!(scale.dtype, "bf16");
        assert_eq!(scale.shape, vec![128, 1]);

        let zero = store
            .meta("model.language_model.layers.0.mlp.gate_proj.weight_int4_zero")
            .expect("CT INT4 implicit-zero alias");
        assert_eq!(zero.dtype, "bf16");
        assert_eq!(zero.shape, vec![128, 1]);

        let view = store
            .int4_storage_view("model.language_model.layers.0.mlp.gate_proj.weight")
            .expect("CT INT4 storage view");
        assert_eq!(view.kind, Int4StorageKind::CtSymmetric);
    ...
    fn mixed_ct_fallback_then_tile_scale_failure_rolls_back_every_publication_map() {
        ...
        let assert_ct_publication =
            |candidate_index: &HashMap<String, TensorMeta>,
             ...
             candidate_views: &HashMap<String, Int4StorageView>| {
                ...
                assert_eq!(alias.byte_len, 4_096);
                assert_eq!(
                ...
                assert!(!candidate_fallbacks.contains_key(TEST_CT_INT4_LOGICAL_NAME));
                let view = candidate_views
                    .get(TEST_CT_INT4_LOGICAL_NAME)
                    .expect("staged CT storage view");
                assert_eq!(view.kind, Int4StorageKind::CtSymmetric);
                assert_eq!(view.group_size, 128);
                assert_eq!(view.implicit_zero_code, Some(8));
            };
        ...
        let mut staged_synthetic = HashMap::new();
        ...
        .expect("publish CT direct INT4 into staged maps");

        assert_ct_publication(
            &staged_index,
            &staged_upload_views,
            &staged_fallbacks,
            &staged_views,
    ...
    #[test]
    fn open_flm_keeps_row_group_storage_view_when_logical_aliases_are_disabled() {
        let fixture = TestRowGroupStage3::rank2();
        let data = build_test_flm_with_stage3_row_group_bindings(&fixture);
        let file = write_temp_flm(&data);
        let store = BakedStore::open_flm_with_options(
        ...
        .expect("open row-group FLM without logical aliases");

        assert!(!store.contains(fixture.logical_name));
        let view = store
            .int4_storage_view(fixture.logical_name)
            .expect("semantic row-group storage view remains available");
        assert_eq!(view.kind, Int4StorageKind::RowGroupSymmetric);
        assert_eq!(view.packed_tensor, fixture.packed_name);
        assert_eq!(view.scale_tensor, fixture.scale_name);
    }
    ...
    #[test]
    fn open_flm_rejects_malformed_row_group_storage_when_aliases_are_disabled() {
        let mut fixture = TestRowGroupStage3::rank2();
        fixture.scale_payload_shape = Some(vec![4, 1]);
        let data = build_test_flm_with_stage3_row_group_bindings(&fixture);
        let file = write_temp_flm(&data);
        let err = match BakedStore::open_flm_with_options(
            file.path(),
            ...
            Ok(_) => panic!("malformed physical scale plane must fail without aliases"),
            Err(err) => err,
        };
    ...
    #[test]
    fn open_flm_resolves_row_group_scale_codec_by_file_local_descriptor() {
        let mut fixture = TestRowGroupStage3::rank2();
        fixture.scale_index_codec = 7;
        let data = build_test_flm_with_stage3_row_group_bindings(&fixture);
        let file = write_temp_flm(&data);
        ...
        .expect("raw BF16 SCALE codec may use any file-local id");

        assert!(store.int4_storage_view(fixture.logical_name).is_some());
    }

    #[test]
    fn open_flm_rejects_row_group_local_codec_zero_remapped_away_from_raw_bf16() {
        let mut fixture = TestRowGroupStage3::rank2();
        fixture.scale_codec_semantic_id = crate::flm::CODEC_FP8_E4M3_F32 as u8;
        let data = build_test_flm_with_stage3_row_group_bindings(&fixture);
        let file = write_temp_flm(&data);
        let err = match BakedStore::open_flm_with_options(
            file.path(),
            ...
            Ok(_) => panic!("local codec id zero is not intrinsically raw"),
            Err(err) => err,
        };

        assert!(
            err.to_string().contains("raw BF16 codec descriptor"),
            "unexpected error: {err}"
        );
    }
    ...
    fn open_flm_rejects_row_group_binding_name_namespace_collisions() {
        ...
        duplicate_physical.include_payloads = false;

        let mut logical_redirects_scale = TestRowGroupStage3::rank2();
        logical_redirects_scale.logical_name = logical_redirects_scale.scale_name;

        for (fixture, expected) in [
            (duplicate_physical, "physical binding names"),
            (
                logical_redirects_scale,
                "logical name must not equal SCALE or ZERO",
            ),
        ] {
            let data = build_test_flm_with_stage3_row_group_bindings(&fixture);
            let file = write_temp_flm(&data);
            let err = match BakedStore::open_flm_with_options(
                ...
                Ok(_) => panic!("binding namespace collision must fail"),
                Err(err) => err,
            };
            assert!(
    ...
    #[test]
    fn open_flm_allows_row_group_logical_name_to_equal_packed_name() {
        let mut fixture = TestRowGroupStage3::rank2();
        fixture.logical_name = fixture.packed_name;
        let data = build_test_flm_with_stage3_row_group_bindings(&fixture);
        let file = write_temp_flm(&data);
        let store = BakedStore::open_flm_with_options(
            file.path(),
        ...
        .expect("producer may use the packed physical name as the logical name");
    ...
    #[test]
    fn open_flm_rejects_row_group_stride_overflow_before_payload_use() {
        let mut fixture = TestRowGroupStage3::rank3();
        fixture.logical_shape = [u32::MAX, u32::MAX, 64, 0];
        fixture.payload_len_override = Some((0, 0));
        let data = build_test_flm_with_stage3_row_group_bindings(&fixture);
        let file = write_temp_flm(&data);
        let err = match BakedStore::open_flm_with_options(
            file.path(),
            ...
            Ok(_) => panic!("full FLM open must reject row-group stride overflow"),
            Err(err) => err,
        };
    ...
    fn flm_runtime_classifies_ct_int4_stage3_plan_as_bf16_fallback() {
        ...
        let extent = store
            ...
            .expect("CT direct storage extent")
            .expect("CT packed UINT8 view is file-backed");
        assert_eq!(extent.upload_dtype, "u8");
        assert_eq!(extent.upload_shape, vec![128, 32]);
        assert_eq!(extent.byte_len, 128 * 32);
    }
    ...
    #[test]
    fn open_flm_materializes_ct_int4_stage3_direct_upload_bytes() {
        let data = build_test_flm_with_stage3_logical_bindings();
        let file = write_temp_flm(&data);
        ...
        let (dtype, shape, bytes) = store
            ...
            .expect("materialize CT INT4 packed UINT8 view");
        assert_eq!(dtype, ScalarType::U8);
        assert_eq!(shape, vec![128, 32]);
        assert_eq!(
            bytes,
            test_ct_int4_packed_bytes(128, 64, |_row, col| (col % 16) as i32 - 8)
        );

        let (zero_dtype, zero_shape, zero_bytes) = store
            .materialize_upload_for_test(
                "model.language_model.layers.0.mlp.gate_proj.weight_int4_zero",
            )
            .expect("materialize implicit CT zero plane");
        assert_eq!(zero_dtype, ScalarType::BF16);
        assert_eq!(zero_shape, vec![128, 1]);
        assert_eq!(zero_bytes, bf16_eight_bytes(256));
    }

    #[test]
    fn open_flm_reserves_virtual_arena_for_ct_int4_stage3_direct_alias() {
        let data = build_test_flm_with_stage3_logical_bindings();
        let file = write_temp_flm(&data);
        ...
        store
            ...
            .expect("direct CT INT4 packed alias is file-backed");
    }
    ...
    #[test]
    fn flm_qwen36_35b_ct_int4_uploads_packed_u8_to_hip() {
        ...
        let meta = store
            .meta(logical_name)
            .expect("CT INT4 logical packed alias");
        assert_eq!(meta.layout, LayoutTag::Int4Quantized);
        assert_eq!(meta.dtype, "u8");
        assert_eq!(meta.shape, vec![8192, 1024]);
        assert!(
            store.raw_bytes(logical_name).is_some(),
            "direct CT INT4 aliases expose packed mmap bytes"
        );

        let weight = store
            .load_to_gpu(logical_name, 0)
            .expect("upload CT INT4 packed UINT8 view");
        assert_eq!(weight.dtype(), ScalarType::U8);
        assert_eq!(weight.shape(), &[8192, 1024]);

        eprintln!("[flm-upload] OK — 35B CT INT4 packed UINT8 view uploaded to HIP");
    }
