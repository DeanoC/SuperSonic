...
fn select_chain_path(persistent: bool, position: PositionPair, expert_prefetch: bool) -> ChainPath {
    match (persistent, position.is_dense(), expert_prefetch) {
...
pub fn run_chain_step(mut args: Qwen36ChainStep<'_>) -> Result<Qwen36ChainStepOutput> {
    ...
    let path = select_chain_path(
        ...
        args.position,
        args.expert_prefetch.is_some(),
    );
    if args.expert_prefetch.is_some() && !args.loaded_layers.has_sparse_expert_residency() {
        anyhow::bail!("Qwen3.6 expert prefetch callback requires sparse expert residency");
    }
