...
//!
//! [`run_batched_prefill`] consumes explicit token and position slices. The
//! caller retains the final prompt token for the first generation step, where
//! logits are computed. Runner translates CLI loop state and experimental
//! sparse-prefill positions into this runtime contract.
//!
...
pub const PREFILL_CHUNK_SIZE_WMMA_FULL: usize = 512;
pub const PREFILL_CHUNK_SIZE_MAX: usize = 1024;
...
pub struct PrefillTokenTimings {
    pub embed: Duration,
    pub chain: Duration,
}
...
/// prefill policy stays outside runtime: runner may compact tokens and supply
/// split RoPE/cache positions, while dense serving supplies
/// [`PositionPair::dense`] values.
#[allow(clippy::too_many_arguments)]
pub fn run_batched_prefill(
    ordinal: usize,
    geom: &MultiLayerGeom,
    store: &BakedStore,
    weight_prefix: &str,
...
fn run_batched_prefill_impl(
    ...
    if tokens.len() != positions.len() {
        return Err(anyhow!(
            "Qwen3.6 prefill token/position length mismatch: {} tokens, {} positions",
            tokens.len(),
            positions.len()
        ));
    }
    ...
    let max_kv_t = loaded_layers
        ...
        .max()
        .unwrap_or(1);
    let max_rope_pos = positions
        .iter()
        .map(|position| position.rope.max(0) as usize + 1)
        .max()
        .unwrap_or(1);
...
fn process_chunk_batched(
    ...
    let split_positions = positions[chunk_start..chunk_start + n]
        .iter()
        .any(|position| !position.is_dense());
    let pos_ids = if split_positions {
        let mut pos_ids_host = Vec::with_capacity(n);
        for slot in 0..n {
            pos_ids_host.push(positions[chunk_start + slot].rope as u32);
        }
...
/// Stage N explicit chunk tokens contiguously on GPU as `[N, hidden]` BF16.
#[allow(clippy::too_many_arguments)]
fn stage_chunk_tokens_on_gpu(
    ...
    for inner in 0..n {
        let token = tokens[chunk_start + inner] as usize;
...
/// Per-token chunked fallback used for FP8 KV/weights or sparse residency.
#[allow(clippy::too_many_arguments)]
...
pub(crate) fn lookup_embed_row(
    ...
    let start = token_id
        ...
        .ok_or_else(|| anyhow!("embed_tokens row offset overflow for token {token_id}"))?;
