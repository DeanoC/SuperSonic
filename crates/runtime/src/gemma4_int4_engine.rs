...
impl Int4Weight {
    fn load(store: &BakedStore, ordinal: usize, base_name: &str) -> Result<Self> {
        ...
        if packed_shape.len() != 2 {
            bail!(
                "{base_name}: packed tensor rank {} != 2",
                packed_shape.len()
            );
        }
        ...
        let zero_shape = zero.shape().to_vec();
        if scale_shape.len() != 2 || zero_shape.len() != 2 {
            bail!(
                "{base_name}: scale/zero rank mismatch (scale={:?} zero={:?})",
                scale_shape,
                zero_shape
            );
        }
        if scale_shape != zero_shape {
            bail!(
                "{base_name}: scale/zero shape mismatch (scale={:?} zero={:?})",
                scale_shape,
                zero_shape
            );
        }
        ...
        if scale_rows == 0 || scale_cols == 0 {
            bail!("{base_name}: invalid empty scale shape {:?}", scale_shape);
        }
        if out_dim % scale_rows != 0 || in_dim % scale_cols != 0 {
            bail!(
                "{base_name}: cannot infer group_size from packed [{out_dim}, {in_dim}] and scale {:?}",
                scale_shape
            );
        }
        let row_group = out_dim / scale_rows;
        let col_group = in_dim / scale_cols;
        if row_group != col_group {
            bail!(
                "{base_name}: non-square INT4 grouping inferred from packed [{out_dim}, {in_dim}] and scale {:?}",
                scale_shape
            );
        }
        Ok(Self {
...
fn load_layer_weights_int4(
    ...
    let (k_proj, v_proj, k_norm) = if shared_kv {
        ...
        (
            Some(Int4Weight::load(
                store,
                ordinal,
                &want("self_attn.k_proj.weight")?,
            )?),
            Some(Int4Weight::load(
                store,
                ordinal,
                &want("self_attn.v_proj.weight")?,
            )?),
    ...
    Ok(Int4LayerWeights {
        ...
        per_layer_input_gate: Int4Weight::load(
            store,
            ordinal,
            &want("per_layer_input_gate.weight")?,
        )?,
        per_layer_projection: Int4Weight::load(
            store,
            ordinal,
            &want("per_layer_projection.weight")?,
        )?,
...
impl Gemma4Int4Engine {
    ...
    pub fn load_with_batch(
        ...
        for i in 0..num_layers {
            layers.push(load_layer_weights_int4(
                &store,
                device,
                &tcfg,
                weight_prefix,
        ...
        let int4_group_size = layers
            .first()
            .map(|l| l.q_proj.group_size)
            .ok_or_else(|| anyhow!("Gemma4Int4Engine: no layers in INT4 bake"))?;
        for (layer_idx, w) in layers.iter().enumerate() {
            let expect = |name: &str, iw: &Int4Weight| -> Result<()> {
                if iw.group_size != int4_group_size {
                    bail!(
                        "layer {layer_idx} {name}: group_size {} != {}",
                        iw.group_size,
                        int4_group_size
                    );
                }
            ...
            expect("q_proj", &w.q_proj)?;
            if let Some(k_proj) = &w.k_proj {
                expect("k_proj", k_proj)?;
            }
            if let Some(v_proj) = &w.v_proj {
                expect("v_proj", v_proj)?;
            }
            expect("o_proj", &w.o_proj)?;
            expect("gate_proj", &w.gate_proj)?;
            expect("up_proj", &w.up_proj)?;
            expect("down_proj", &w.down_proj)?;
            expect("per_layer_input_gate", &w.per_layer_input_gate)?;
            expect("per_layer_projection", &w.per_layer_projection)?;
        }

        let lm_head_w =
            store.load_to_gpu(&format!("{weight_prefix}.embed_tokens.weight"), device)?;
        let final_norm_w = store.load_to_gpu(&format!("{weight_prefix}.norm.weight"), device)?;
        ...
        let intermediate_max = layers
            .iter()
            .map(|l| l.intermediate_size)
            .max()
            .unwrap_or(0);
        ...
        for l in 0..num_layers {
            let src_idx = if layers[l].shared_kv {
                layers[l].kv_source
            } else {
                l
            };
    ...
    pub fn restore_prefix(&mut self, snapshot: &Gemma4Int4EngineSnapshot) -> Result<Vec<f32>> {
        ...
        for (i, src) in snapshot.k_caches.iter().enumerate() {
            ...
            gpu_hal::copy_d2d(self.device, dst.as_mut_ptr(), src.as_ptr(), src.len_bytes())
                .with_context(|| format!("restore Gemma 4 INT4 K prefix layer {i}"))?;
        }
        for (i, src) in snapshot.v_caches.iter().enumerate() {
            ...
            gpu_hal::copy_d2d(self.device, dst.as_mut_ptr(), src.as_ptr(), src.len_bytes())
                .with_context(|| format!("restore Gemma 4 INT4 V prefix layer {i}"))?;
        }
    ...
    fn compute_per_layer_inputs_batched(
        ...
        let mut proj_normed = GpuBuffer::zeros(device, dtype, &[seq_len, num_layers, ple_hidden])?;
        g4::rms_norm_rows(
            device,
            dtype,
            &mut proj_normed,
    ...
    fn prefill_inner(
        ...
        let int4_group_size = self.int4_group_size;
        let eps = self.tcfg.rms_norm_eps as f32;
        let ple_hidden = self.tcfg.hidden_size_per_layer_input;
        let num_layers = self.tcfg.num_hidden_layers;
        ...
        for layer_idx in 0..num_layers {
            ...
            let mut q_normed = GpuBuffer::zeros(device, dtype, &[seq_len, num_q_heads, head_dim])?;
            g4::rms_norm_rows(
                device,
                dtype,
                &mut q_normed,
            ...
            let mut attn_out = GpuBuffer::zeros(device, dtype, &[seq_len, num_q_heads, head_dim])?;
            let mut scores =
            ...
            g4::rms_norm_rows(
                ...
                &mut normed,
                &projected,
                Some(&w.post_per_layer_input_norm),
                eps,
                seq_len,
                hidden_size,
            )?;
            ...
            if capture {
                per_layer_capture.push(extract_last_row_bf16(
                    device,
                    &h_running,
                    seq_len,
                    hidden_size,
    ...
    pub fn decode_step_batch(
        ...
        for (b, &pos) in positions.iter().enumerate() {
            if pos >= self.max_t {
                bail!(
                    "decode_step_batch: seq {b} pos {pos} >= max_t {}",
                    self.max_t
                );
            }
        }
        ...
        let pli_gpu =
            GpuBuffer::from_host_bytes(device, dtype, &[b, num_layers, ple_hidden], &pli_bytes)?;
    ...
    #[allow(dead_code)]
    fn _decode_step_fused_per_layer(
        &mut self,
        input_token_id: u32,
        pos: usize,
    ) -> Result<Vec<f32>> {
        ...
        for layer_idx in 0..num_layers {
            ...
            g4::fused_attn_block_int4(
                ...
                cos_table,
                sin_table,
                &mut k_caches_src[layer_idx],
                &mut v_caches_src[layer_idx],
                &mut self.fused_workspace,
                &mut self.fused_matvec_counter,
                &mut self.fused_barrier_counter,
                &mut self.fused_barrier_flag,
    ...
    /// the attention block.
    pub fn decode_step_primitive(&mut self, input_token_id: u32, pos: usize) -> Result<Vec<f32>> {
        if pos >= self.max_t {
