...
fn main() -> Result<()> {
    ...
    // Teacher-forced is currently implemented for Llama 3.1, Qwen3.5, and Gemma 4.
    // Bail early for unsupported families so the user gets a clear message.
    if cli.teacher_forced {
        match model_variant.family() {
            ModelFamily::Llama31 | ModelFamily::Qwen35 | ModelFamily::Gemma4 => {} // supported; handled inside engine
            ModelFamily::Phi4 => {
