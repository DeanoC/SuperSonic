//! End-to-end greedy decode validator for Gemma 4 E2B.
//!
//! This binary closes the loop on the Rust-side primitive kernels: given a
//! PyTorch oracle dump (`oracle/gemma4_oracle.py --emit-state`), it seeds the
//! per-layer K/V caches from the oracle's prefill, then generates N tokens
//! with the same kernels `gemma4_layer0_validate` uses, comparing softcapped
//! logits at every step against `oracle.prefill_logits` / `oracle.decode_logits`
//! and the final generated IDs against `oracle.generated_token_ids`.
//!
//! Step 0 reproduces the "last prompt token" forward (input = scaled embedding
//! of `prompt_token_ids[-1]`, pos = `prompt_tokens - 1`), matching
//! `gemma4_layer0_validate --chain`. Steps 1..=N-1 are true decode steps:
//! the input embedding is `argmax` of the previous step's softcapped logits
//! and pos is `prompt_tokens - 1 + step`. This exercises `kv_append` at
//! multiple distinct positions — previous sessions only appended once per run.
//!
//! Shared-KV layers (the last `num_kv_shared_layers` layers on E2B, which is
//! layers 15..=34) don't project their own K/V. They read from an earlier
//! layer's K/V cache at HF runtime via pointer aliasing; here, because every
//! layer owns a distinct device buffer, we replicate each non-shared layer's
//! freshly-appended slot into every shared layer that points back to it,
//! immediately after `kv_append`.
//!
//! Usage:
//!   cargo run --release --bin gemma4_decode_validate -- \
...
fn main() -> Result<()> {
    ...
    for step in 0..max_new_tokens {
        ...
        for layer_idx in 0..num_layers {
            ...
            g4::matvec(0, dtype, &mut q, &x, &w.q_proj, hidden_size, q_dim, &mut counter)?;
            ...
            if !w.shared_kv {
                ...
                // Replicate slot `pos` to every shared layer that sources from
                // this one. Source->shared indices monotonically increase so
                // dependents processed later in this same step iteration will
                // see the fresh slot.
                for shared_layer in (layer_idx + 1)..num_layers {
                    let s = &layers[shared_layer];
                    if s.shared_kv && s.kv_source == layer_idx {
                        // Split-borrow the caches vector to copy between two entries.
            ...
            g4::matvec(0, dtype, &mut o, &attn_flat, &w.o_proj, q_dim, hidden_size, &mut counter)?;
