...
fn main() -> Result<()> {
    ...
    let root =
        PathBuf::from(std::env::var("INT4_DIAG_DIR").unwrap_or_else(|_| "/tmp/int4_diag".into()));
    ...
    let hidden_gpu =
        GpuBuffer::from_host_bytes(ordinal, ScalarType::BF16, &[meta.m, meta.k], &hidden_bytes)
            .map_err(|e| anyhow!("hidden upload: {e}"))?;
    let rhs_gpu = GpuBuffer::from_host_bytes(
        ordinal,
        ScalarType::U8,
        &[meta.n, meta.k_packed],
        &packed_bytes,
    )
    ...
    let scale_gpu = GpuBuffer::from_host_bytes(
        ordinal,
        ScalarType::BF16,
        &[meta.scale_rows, meta.scale_cols],
        &scale_bytes,
    )
    ...
    let out_host = bf16_bytes_to_f32(&out_gpu.to_host_bytes().map_err(|e| anyhow!("d2h: {e}"))?);
    ...
    let rmse = ((sum_sq_g + sum_sq_r
        - 2.0 * {
            let mut s = 0f64;
            for i in 0..nref {
                s += out_host[i] as f64 * ref_host[i] as f64;
            }
            s
        })
        / (nref as f64))
    ...
    println!(
        "[real-test] avg |gpu| = {:.4}, avg |ref| = {:.4}",
        sum_abs_g / nref as f64,
        sum_abs_r / nref as f64
    );
    println!(
        "[real-test] rms(ref) = {:.4},  rms(gpu) = {:.4},  rmse = {:.6}",
        (sum_sq_r / nref as f64).sqrt(),
        (sum_sq_g / nref as f64).sqrt(),
        rmse
    );
    ...
    println!(
        "[real-test] samples: gpu[0,0]={:.4}  ref[0,0]={:.4}",
