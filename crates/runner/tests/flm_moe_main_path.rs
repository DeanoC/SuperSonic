...
fn assert_moe_flm_main_path_contract(combined: &str) {
    ...
    assert_eq!(
        occurrence_count(combined, "[qwen36-moe] runtime engine ready:"),
        1,
        "FLM main path should construct the runtime engine exactly once:\n{combined}"
    );
    assert!(
        combined.contains("[qwen36-moe] runtime engine ready: load_sequence=1 source_open_count=1"),
        "runtime engine did not own exactly one observed FLM source open:\n{combined}"
    );
    assert!(
