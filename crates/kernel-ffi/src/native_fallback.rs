...
unsafe extern "C" {
    ...
    fn supersonic_metal_qwen36_batched_ffn_grouped_expert_direct_with_options(
        n_tokens: usize,
        top_k: usize,
        hidden: usize,
        moe_intermediate: usize,
        ...
        expert_mid_ptr: *mut c_void,
        combined_ptr: *mut c_void,
        profile_enabled: c_int,
        phase_profile_enabled: c_int,
        wait_for_completion: c_int,
    ) -> c_int;
...
impl MetalBatchGuard {
    pub(crate) fn begin() -> Result<Self, GpuError> {
        ...
        if status != 0 {
            return Err(GpuError::backend(
                Backend::Metal,
                format!("metal native batch begin failed with status {status}"),
            ));
        }
        Ok(Self { active: true })
    }
    ...
    pub(crate) fn finish(mut self) -> Result<(), GpuError> {
        ...
        if status != 0 {
            return Err(GpuError::backend(
                Backend::Metal,
                format!("metal native batch end failed with status {status}"),
            ));
        }
        Ok(())
    }
...
pub(crate) fn flush_batch() -> Result<(), GpuError> {
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native batch flush failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn set_batch_label(label: &str) -> Result<(), GpuError> {
    let label = CString::new(label).map_err(|_| {
        GpuError::backend(
            Backend::Metal,
            "metal native batch label contains NUL byte".to_string(),
        )
    })?;
...
pub(crate) fn copy_d2d(src: *const c_void, dst: *mut c_void, bytes: usize) -> Result<(), GpuError> {
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native copy_d2d failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn lm_head_argmax_bf16_with_partials(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native lm_head_argmax_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn argmax_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native argmax_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn embedding_lookup_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native embedding_lookup_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn matmul_rhs_transposed_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native matmul_rhs_transposed_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn matmul_rhs_transposed_bf16_gemv_m1(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native matmul_rhs_transposed_bf16_gemv_m1 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn matmul_rhs_transposed_residual_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native matmul_rhs_transposed_residual_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn matmul_rhs_transposed_int4_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native matmul_rhs_transposed_int4_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) unsafe fn qwen36_batched_prefill_grouped_expert_direct_with_options(
    ...
    {
        return Err(GpuError::InvalidArg(format!(
            "metal native qwen36_batched_prefill_grouped_expert_direct_with_options invalid shape: n_tokens={n_tokens} top_k={top_k} hidden={hidden} moe_intermediate={moe_intermediate} group_size={group_size}"
        )));
    }
    let status = unsafe {
        supersonic_metal_qwen36_batched_ffn_grouped_expert_direct_with_options(
            n_tokens,
            top_k,
            hidden,
            moe_intermediate,
            ...
            expert_mid,
            combined,
            i32::from(profile_enabled),
            i32::from(phase_profile_enabled),
            i32::from(wait_for_completion),
        )
    };
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!(
                "metal native qwen36_batched_prefill_grouped_expert_direct_with_options failed with status {status}"
            ),
        ));
    }
    Ok(())
...
pub(crate) fn matmul_rhs_transposed_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native matmul_rhs_transposed_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_linear_projections_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_linear_projections_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_mlp_gate_up_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_mlp_gate_up_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_mlp_gate_up_swiglu_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_mlp_gate_up_swiglu_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_mlp_down_residual_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_mlp_down_residual_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_linear_out_residual_f32_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_linear_out_residual_f32_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_linear_out_residual_bf16_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_linear_out_residual_bf16_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_full_projections_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_full_projections_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn full_attention_prefill_strided_bf16_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native full_attention_prefill_bf16_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn full_attention_decode_bf16_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native full_attention_decode_bf16_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn rms_norm_rows_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native rms_norm_rows_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn rms_norm_rope_rows_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native rms_norm_rope_rows_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn rms_norm_rows_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native rms_norm_rows_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn rms_norm_gated_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native rms_norm_gated_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn rms_norm_gated_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native rms_norm_gated_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn linear_prefill_conv_pack_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native linear_prefill_conv_pack_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn l2norm(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native l2norm failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn full_attention_gate_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native full_attention_gate_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn swiglu_mul(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native swiglu_mul failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn cast(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native cast failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn mul_scalar(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native mul_scalar failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn transpose_shd_hsd(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native transpose_shd_hsd failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn apply_rope_prefill(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native apply_rope_prefill failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn transpose_pad_conv(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native transpose_pad_conv failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn extract_conv_state(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native extract_conv_state failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn split_qkv(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native split_qkv failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn split_qgate(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native split_qgate failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn repeat_interleave_heads(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native repeat_interleave_heads failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn compute_beta_g_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native compute_beta_g_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn delta_recurrent_prefill_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native delta_recurrent_prefill_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn linear_decode_apply_parts_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native linear_decode_apply_parts_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn qwen_linear_prep_bf16_f32(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native qwen_linear_prep_bf16_f32 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn conv_state_update_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native conv_state_update_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn linear_conv_value_decay_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native linear_conv_value_decay_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) fn linear_conv_value_decay_update_bf16(
    ...
    if status != 0 {
        return Err(GpuError::backend(
            Backend::Metal,
            format!("metal native linear_conv_value_decay_update_bf16 failed with status {status}"),
        ));
    }
    Ok(())
}
...
pub(crate) unsafe fn qwen36_batched_prefill_grouped_expert_direct_with_options(
    ...
    _phase_profile_enabled: bool,
    _wait_for_completion: bool,
) -> Result<(), GpuError> {
    Err(GpuError::backend(
        Backend::Metal,
        "metal native qwen36_batched_prefill_grouped_expert_direct_with_options is not compiled"
            .into(),
    ))
}
...
pub(crate) fn cast(
    ...
    Err(GpuError::backend(
        Backend::Metal,
        "metal native cast is not compiled".into(),
    ))
}
