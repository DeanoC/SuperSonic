...
| Model | Option | ms/tok | tok/s |
|---|---|---:|---:|
| qwen3.5-35b-a3b | Q4_K_M, 1-token prompt + 512 generated tokens | 67.3 | 14.9 |
...
Public reference numbers for the same Qwen3.5-35B-A3B Q4_K_M target on M5
Max are llama.cpp at 91 tok/s (~11.0 ms/tok) and MLX at 139 tok/s
(~7.2 ms/tok). The SuperSonic Metal path is currently 16% of the llama.cpp
throughput on the 512-token generation checkpoint above.
...
tracked in [supported-matrix.md](supported-matrix.md); Qwen3.5 and Qwen3.6
are the currently published large-model headline performance rows.
