...
| --- | --- | --- | --- | --- | --- |
| `qwen3.8-27b` | custom `gqh-gguf` | `gfx1100` | `experimental` | single-sequence greedy, optional NextN/MTP or DFlash2 | `qwen38-gqh-correctness` |
| `qwen3.8-27b` | custom `gqh-gguf` | `gfx1201` | `experimental` | single-sequence greedy, optional NextN/MTP or DFlash2 | `qwen38-gqh-correctness` |
...
`HIP_ARCH=gfx1201`, checks the configured artifact pair, and compares the
ordinary and NextN/MTP token arrays. DFlash2 uses the int8 serving policy and is
gated by semantic quality cases rather than ordinary-token equality.

The R9700 lane serves with the NextN/MTP and width-16 DFlash2 int8 verify arms
enabled. The fused-matvec path quantizes activations to int8 and runs a dp4a
dot against a baked int8 weight LUT instead of the f32 matvec. The arm declines
when a rung's level grid does not fit int8's range or the input dim is not a
256-element superblock multiple. `GGML_GQH_I8DOT=0` remains an explicit measured
control, not a serving mode. The MTP support gate still requires int8
ordinary-versus-MTP token equality through
`mtp_i8_verify_token_equality_matches_greedy`.
