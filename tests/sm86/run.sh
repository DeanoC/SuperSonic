...
# Runs Qwen3.5-0.8B decode with CUDA oracle validation and checks max delta.
#
# Usage:
#   ./tests/sm86/run.sh [model_dir]
#
set -euo pipefail
...
if [ -f /root/.cargo/env ]; then
    # Allow the script to run on fresh boxes where rustup is installed but not sourced.
    . /root/.cargo/env
fi
MAX_DELTA_THRESHOLD="${MAX_DELTA_THRESHOLD:-1.0}"
GPU_VALIDATE_DELTA_THRESHOLD="${GPU_VALIDATE_DELTA_THRESHOLD:-0.25}"
TIMEOUT="${TIMEOUT:-180}"
PROMPT="Hello"
MAX_NEW_TOKENS=4
TMPOUT=$(mktemp)
...
echo "=== SuperSonic E2E Test: sm86 / qwen3.5-0.8b ==="
