#!/usr/bin/env bash
#
# Warmed Meta-Llama-3.1-8B CUDA sm86 single-sequence benchmark.
# Measures the shipping BF16 raw-safetensors path with optional stage timings.
#
# Usage:
#   ./tests/sm86/bench_llama31_8b.sh [model_dir]
#
# Environment:
#   TARGET_PROMPT_TOKENS=520  approximate prompt length target
#   MAX_NEW_TOKENS=128        generated tokens per timed run
#   WARMUP_RUNS=5             untimed warmup runs
#   TIMED_RUNS=10             timed runs to average
...
echo "=== SuperSonic sm86 Meta-Llama-3.1-8B Benchmark ==="
...
stage_pat = re.compile(
    ...
    r"persistent=([0-9.]+) rms_norm=([0-9.]+) lm_head=([0-9.]+) "
    r"logits_d2h=([0-9.]+) host_sampling=([0-9.]+)"
)

base = "SuperSonic CUDA benchmark context for Meta Llama three point one decode throughput. "
...
def run_once(prompt: str, run_max_new_tokens: int):
    ...
    return {
        ...
        "persistent_ms": float(stage.group(4)) if stage else 0.0,
        "rms_norm_ms": float(stage.group(5)) if stage else 0.0,
        "lm_head_ms": float(stage.group(6)) if stage else 0.0,
        "logits_d2h_ms": float(stage.group(7)) if stage else 0.0,
        "host_sampling_ms": float(stage.group(8)) if stage else 0.0,
    }
...
print("")
print("--- llama3.1-8b native decode lane ---")
...
print(
    "stage_ms_mean "
    f"stage_tokens={stage_tokens} total={stage_total_mean:.3f} persistent={persistent_mean:.3f} "
    f"rms_norm={rms_mean:.3f} lm_head={lm_head_mean:.3f} logits_d2h={logits_d2h_mean:.3f} "
    f"host_sampling={host_sampling_mean:.3f}"
)
PY
