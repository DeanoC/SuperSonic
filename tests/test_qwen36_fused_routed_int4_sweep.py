...
class Qwen36FusedRoutedInt4SweepTests(unittest.TestCase):
    ...
    def test_build_env_overrides_for_fused_modes(self):
        ...
        full_stage5_router_simd_batch_shared_mid_host_corrected = script.build_env_overrides(
            args,
            "full-stage5-router-simd-batch-shared-mid-host-corrected",
        )
        full_stage5_router_simd_batch_shared_mid_routed_down_recomputed = (
            script.build_env_overrides(
                args,
                "full-stage5-router-simd-batch-shared-mid-routed-down-host-recomputed",
            )
        )
        full_stage5_router_simd_batch_shared_mid_routed_down_recomputed_layer33 = (
            script.build_env_overrides(
                args,
                "full-stage5-router-simd-batch-shared-mid-routed-down-host-recomputed-layer33",
            )
        ...
        self.assertEqual(
            full_stage5_router_simd_batch_shared_mid_host_corrected[
                "SUPERSONIC_METAL_ENABLE_QWEN36_FFN_ROUTER_STAGE5_SIMD"
            ],
            "1",
        )
    ...
    def test_build_command_omits_stage_timing_for_batch_mode(self):
        ...
        simd_batch_shared_mid_host_corrected = script.build_command(
            args,
            "Hello",
            "full-stage5-router-simd-batch-shared-mid-host-corrected",
        )
        simd_batch_shared_mid_routed_down_recomputed = script.build_command(
            args,
            "Hello",
            "full-stage5-router-simd-batch-shared-mid-routed-down-host-recomputed",
        )
        simd_batch_shared_mid_routed_down_recomputed_layer33 = script.build_command(
            args,
            "Hello",
            "full-stage5-router-simd-batch-shared-mid-routed-down-host-recomputed-layer33",
        )
        ...
        self.assertNotIn("--emit-stage-timings", simd_batch_shared_mid_host_corrected)
        self.assertNotIn("--emit-stage-timings", simd_batch_shared_mid_routed_down_recomputed)
        self.assertNotIn(
            "--emit-stage-timings",
    ...
    def test_source_aware_layer_output_tolerance_requires_allowed_boundary(self):
        ...
        self.assertEqual(
            report["summary"]["drift_containment"]["recommendation"],
            "drift_contained_by_boundary_policy",
        )
        self.assertEqual(
            report["summary"]["drift_containment"]["contained_modes"],
            ["direct-gather"],
        )
    ...
    def test_drift_containment_reports_first_parity_expansion_layer(self):
        script = sweep_qwen36_fused_routed_int4
        baseline = row("default", ids=[11])
        candidate = row(
            "full-stage5-router-simd-batch",
            ids=[11],
            headline=90.0,
        ...
        containment = report["summary"]["drift_containment"]
        prompt = containment["prompt_results"][0]
        md = script.render_markdown(report)

        self.assertEqual(
            containment["recommendation"],
            "bisect_first_parity_expansion_layer",
        )
        self.assertEqual(
            containment["modes_with_expansion"],
            ["full-stage5-router-simd-batch"],
        )
        self.assertEqual(prompt["status"], "expanded")
        self.assertEqual(prompt["first_allowed_boundary"]["layer"], 7)
        self.assertEqual(prompt["first_parity_expansion"]["layer"], 8)
        self.assertEqual(prompt["layers_after_boundary"], 1)
        self.assertIn("drift_containment_recommendation: `bisect_first_parity_expansion_layer`", md)
        self.assertIn("## Drift Containment", md)
    ...
    def test_shared_mid_host_correction_rows_are_parsed_and_rendered(self):
        script = sweep_qwen36_fused_routed_int4
        output = (
            "[qwen36-ffn-shared-mid-host-correction] layer=7 shared_path=host_order "
            "hidden=2048 shared_intermediate=768 "
            "shared_mid_max_abs=5.96046448e-08 shared_mid_argmax=13 "
            "host_shared_mid_at_argmax=3.36338878e-01 "
            "metal_shared_mid_at_argmax=3.36338818e-01 "
            "shared_out_patch_max_abs=6.10351562e-05 shared_out_patch_argmax=1621 "
            "host_shared_out_at_argmax=1.23901367e-02 "
            "metal_shared_out_at_argmax=1.23291016e-02 "
            "output_patch_max_abs=4.88281250e-04 output_patch_argmax=1621 "
            "changed_shared_mid_elems=1 changed_shared_out_elems=1 "
            "changed_output_elems=1 first_changed_output=1621\n"
        )

        corrections = script.parse_shared_mid_host_corrections(output)

        self.assertEqual(len(corrections), 1)
        self.assertEqual(corrections[0]["layer"], 7)
        self.assertEqual(corrections[0]["shared_path"], "host_order")
        self.assertEqual(corrections[0]["shared_mid_argmax"], 13)
        self.assertAlmostEqual(corrections[0]["shared_mid_max_abs"], 5.96046448e-08)
        self.assertEqual(corrections[0]["changed_shared_mid_elems"], 1)
        ...
        mode = "full-stage5-router-simd-batch-shared-mid-host-corrected"
        candidate = row(mode)
        candidate["shared_mid_host_corrections"] = corrections
        report = script.build_report(
            [row("default"), candidate],
            args,
            ["default", mode],
        ...
        self.assertEqual(
            report["summary"]["shared_mid_host_correction"]["correction_count"],
            1,
        )
        self.assertEqual(report["summary"]["shared_mid_host_correction"]["changed_count"], 1)
        self.assertIn("shared_mid_host_correction_count: `1`", md)
        self.assertIn(
            "shared_mid_host_correction_max_shared_mid_abs: `0.00000006`",
            md,
        )
        self.assertIn("## Shared-Mid Host Correction", md)
        self.assertIn(
            "| hello | full-stage5-router-simd-batch-shared-mid-host-corrected | host_order | 7 |",
            md,
        )
    ...
    def test_routed_down_host_recompute_correction_rows_are_parsed_and_rendered(self):
        script = sweep_qwen36_fused_routed_int4
        output = (
            "[qwen36-ffn-routed-down-host-recompute-correction] layer=33 "
            "router_path=simd hidden=2048 top_k=8 topk_idx_match=1 "
            "topk_weight_max_abs=0.00000000e+00 topk_weight_argmax=0 "
            "expert_mid_max_abs=1.19209290e-07 expert_mid_argmax=12 "
            "expert_mid_group=0 expert_mid_row=12 "
            "host_expert_mid_at_argmax=1.00000000e+00 "
            "metal_expert_mid_at_argmax=9.99999881e-01 "
            "metal_moe_vs_recomputed_max_abs=1.22070312e-04 "
            "metal_moe_vs_recomputed_argmax=8 "
            "recomputed_down_acc_at_moe_argmax=-2.19726562e-02 "
            "recomputed_moe_at_moe_argmax=-2.19726562e-02 "
            "host_moe_at_moe_argmax=-2.19726562e-02 "
            "metal_moe_at_moe_argmax=-2.20947266e-02 "
            "host_moe_vs_recomputed_max_abs=0.00000000e+00 "
            "host_moe_vs_recomputed_argmax=0 "
            "recomputed_moe_at_host_argmax=1.00000000e-03 "
            "host_moe_at_host_argmax=1.00000000e-03 "
            "metal_moe_at_host_argmax=1.00000000e-03 "
            "output_patch_max_abs=1.22070312e-04 output_patch_argmax=8 "
            "final_vs_host_max_abs=0.00000000e+00 final_vs_host_argmax=0 "
            "host_final_at_final_argmax=1.26953125e-02 "
            "recomputed_final_at_final_argmax=1.26953125e-02 "
            "changed_moe_elems=1 changed_output_elems=1 first_changed_output=8\n"
        )

        corrections = script.parse_routed_down_host_recompute_corrections(output)

        self.assertEqual(len(corrections), 1)
        self.assertEqual(corrections[0]["layer"], 33)
        self.assertEqual(corrections[0]["router_path"], "simd")
        self.assertEqual(corrections[0]["metal_moe_vs_recomputed_argmax"], 8)
        self.assertAlmostEqual(
            corrections[0]["metal_moe_vs_recomputed_max_abs"],
            0.000122070312,
        )
        ...
        mode = "full-stage5-router-simd-batch-shared-mid-routed-down-host-recomputed"
        candidate = row(mode)
        candidate["routed_down_host_recompute_corrections"] = corrections
        report = script.build_report(
            [row("default"), candidate],
            args,
            ["default", mode],
        ...
        summary = report["summary"]["routed_down_host_recompute_correction"]
        self.assertEqual(summary["correction_count"], 1)
        self.assertEqual(summary["changed_count"], 1)
        self.assertAlmostEqual(summary["max_metal_moe_vs_recomputed_abs"], 0.000122070312)
        self.assertIn("routed_down_host_recompute_correction_count: `1`", md)
        self.assertIn(
            "routed_down_host_recompute_correction_max_metal_moe_vs_recomputed_abs: `0.00012207`",
            md,
        )
